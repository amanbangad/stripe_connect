...
const mockStripe = {
  ...
  accounts: {
    ...
    // Attaching an external account + KYC completes onboarding; in the mock we
    // simply record the bank token and flip the transfers capability active.
    async update(courierId, params = {}) {
      const acct = accounts.get(courierId);
      if (!acct) {
        const err = new Error(`No such account: ${courierId}`);
        err.statusCode = 404;
      ...
      if (params.external_account) acct.external_account = params.external_account;
